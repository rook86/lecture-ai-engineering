...
### llm_analysis（新たに実装したもの）
Streamlitを使用したLLM（大規模言語モデル）ベースのデータ分析アプリケーション

- **`app.py`**: FastAPIを使用してLLMモデルを提供するAPIサーバー。モデルのロード、データ分析を行う。
- **`ui.py`**: アプリケーションのUIロジックを管理する。
- **`llm.py`**: LLMモデルのロードとデータ分析を行うモジュール。
- **`requirements.txt`**: このアプリケーションを実行するために必要なPythonパッケージ。
